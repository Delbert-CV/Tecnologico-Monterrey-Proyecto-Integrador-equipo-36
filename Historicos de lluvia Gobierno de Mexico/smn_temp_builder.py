...
for type_of_temp in available_temps:
    ...
    for year in years:
        for month in months:
            path_to_load = os.path.join(data_dir, f"{year}_{month:02d}_000_{type_of_temp}.csv")
            
            
            if os.path.exists(path_to_load):
                df = pd.read_csv(path_to_load, encoding='latin1', usecols=range(6), engine='python')
                df.columns = ['lon', 'lat', 'state', 'cve_sih', 'name', f'{type_of_temp}'] 
    ...
    df_final.to_csv(f'{type_of_temp}_per_area_historical.csv', index=False)
